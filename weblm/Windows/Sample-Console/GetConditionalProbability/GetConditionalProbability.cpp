// GetConditionalProbability.cpp : Defines the entry point for the console application.
//
...
int wmain(int argc, wchar_t *argv[], wchar_t *envp[])
{
	...
	client.calculateConditionalProbability(args.get(L"model"), args.get(L"order"), args.phrases())
		.then([=](json::value v)
		{
			...
			for (auto result = results.cbegin(); result != results.cend(); result++)
			{
				...
				wcout << L" :: " << result->at(L"word").as_string();
